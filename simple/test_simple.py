...
def test_simple_list_keys():
    ...
    for person in persons:
        ...
        assert (
            len(keys_missing) == 0
        ), f"Missing keys {keys_missing} in person: {person}"
...
def test_simple_list_age():
    ...
    for person in persons:
        assert isinstance(
            person["age"], int
        ), f"Expected 'age' to be int but got {type(person['age'])}"
        assert (
            1 <= person["age"] <= 100
        ), f"Expected 'age' to be 1-100 but got {person['age']}"
...
def test_sort_list():
    ...
    for x in range(len(persons) - 1):
        ...
        ), f"List is not properly sorted at index {x}"
